...
The part of the chain share that goes to earlier coins is split by position in a U shape. \$DOLL, the oldest coin, always gets the largest part, about twice the newest link's. The newest links get the next largest parts, and the coins in the middle get the least.

For a holder that means two things. The oldest coins are the foundation every trade passes through, so holding them keeps paying as the chain grows. The newest links sit where the current round's activity is, so holding them pays while trading is near them. The exact weights are in [The numbers](/the-chain#the-numbers).
</Accordion>
