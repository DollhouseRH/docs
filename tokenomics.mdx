...
Any rounding remainder left over after the curve is filled is burned. The curve has the same shape for every coin. Four ranges sell 20%, 25%, 35% and 20% of the supply, across price bands that run from one thousandth of the parent's supply value up to 100x it, with the last range as a tail.

\$DOLL is launched in its own market outside Dollhouse; see [\$DOLL](/the-genesis-coin). The developer holds 3% of \$DOLL, 30,000,000 tokens, bought in the launch transaction. They are locked in a public vesting contract that releases them evenly over one year, starting at launch. The vesting contract's address and the address that bought the tokens are published on this page.
...
- Liquidity, the money and coins in a pool, stays in its pool for good. The contracts only add to it.
- The fee, the shares, the curve, the round timings and the bond are fixed in this deployment.
- Round winners are decided by scoring alone, and every link, once written, stays as written.
